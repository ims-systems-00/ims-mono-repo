...
const { incidentsByStatus } = require("../charts");
...
async function pageFour(doc, data) {
  ...
  doc.addPage({ size: "A4" });
  dox = sectionHeader(doc, "Incident data based on their status");
  let incidentsByStatusChart = await incidentsByStatus(data);
  doc.image(incidentsByStatusChart, 60, 120, {
