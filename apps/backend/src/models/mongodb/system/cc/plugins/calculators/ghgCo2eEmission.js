...
async function calc() {
  ...
  try {
    ...
    if (
      ...
      const factor =
        customFactor && customFactor["ghgConversionFactor"]
          ? customFactor["ghgConversionFactor"]
          : 0;
      console.log("customFactor", factor);
      this.ghgCo2eEmission = (this.amount * factor) / 1000;
      this.ghgCo2eEmission = this.ghgCo2eEmission.toFixed(5);
      this.activity = customFactor && customFactor.combinedActivityReference;
      this.unit = customFactor && customFactor.uom;
    }
    if (
      [
        CC_EMISSION_CATEGORY_NAMES.COMPANY_PREMISES,
    ...
    if (
      ...
      const factor =
        sicFactor && sicFactor["sicGhgCo2ePerCurrency"]
          ? sicFactor["sicGhgCo2ePerCurrency"]
          : 0;
      this.ghgCo2eEmission = (this.amount * factor) / 1000;
      this.ghgCo2eEmission = this.ghgCo2eEmission.toFixed(5);
    }
