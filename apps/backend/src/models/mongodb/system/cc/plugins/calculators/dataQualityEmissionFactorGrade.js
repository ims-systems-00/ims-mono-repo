...
async function calc(next) {
  ...
  try {
    ...
    if (
      ...
      const factor =
        customFactor && customFactor["grade"]
          ? customFactor["grade"]
          : CC_DATA_QUALITY_GRADES.FAIR;
      this.dataQualityEmissionFactorGrade = factor;
    }
    if (
      [
        CC_EMISSION_CATEGORY_NAMES.COMPANY_PREMISES,
...
module.exports = (schema) => {
  schema.add({
    dataQualityEmissionFactorGrade: {
      type: String,
      default: null,
    },
  });
