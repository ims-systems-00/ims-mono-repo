    ...
    if (
      ...
      if (
        ...
        const defraFactor = await CCFactors.findOne({
          ...
          uom: this.unit,
          ghgPerUnit: CC_GHG_GASSES.N2O,
        });
        const factor =
          defraFactor && defraFactor["ghgConversionFactor"]
            ? defraFactor["ghgConversionFactor"]
            : 0;
        this.ghgN2oEmission = (this.amount * factor) / 1000;
        this.ghgN2oEmission = this.ghgN2oEmission.toFixed(5);
        return next();
      } else {
        this.ghgN2oEmission = 0;
      }
    }
    if (
      [
      ...
      REFIRGERANT_GHG_MAPS[this.activity] === CC_GHG_GASSES.N2O
    ) {
      this.ghgN2oEmission = this.ghgCo2eEmission;
    } else this.ghgN2oEmission = 0;
  } catch (err) {
