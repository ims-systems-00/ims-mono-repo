# v4.0.0 ( 05 February 2024 )

## Added

### Onboarding

- Seamless iMS partner onboarding
- Seamless iMS customer onboarding

### Automated billing system

- Customers can manage licenses and add credit cards to pay for it.

# v3.0.12 ( 27 November 2023 )

## Added

### Custom Category

- User can now add category to incident,risk,crm,inventory based on their requirements
- Categories are accessible through the button on top of each table

### Advanced Filter

- An advanced filter ability with multiple condition checking is introduced

### Document Management

- Resend of request resend implemented

### Document linking

- Document linking to iso toolkit is introduced
- While uploading a document user get to select if the document needs to be linked with any toolkit

# v3.0.11 ( 16 October 2023 )

## Added

### Dashboard

- Charts and graphs been updated.
- More insight and modern designs are there

### User profile

- New UI implemented in user profile section
- UX been improved in terms of handling user

## Bug fixes

### Document linking

- Document fetching issue in modules are sorted
...
Drawer component integrated into UI kit. Future development should import drawer components and hooks from ims-ui-kit. Existing components requires replacement as soon as possible. Relavant branch for this refactor is feat/drawer-component-replacement

# v3.0.10 ( 17 August 2023 )
...
- Audit analysis on a perticular audit.
...
Follwoing features are labeled beta.

- Risk management has now risk analysis of each risk.
- Incident management has now incident analysis of each inciden.

Analyser buttons are attached at the top of drawer details view.

### Automated activity logs

Captured automated activities on the folliwing topics

- When an OFI is created.
- When an OFI is implemented.
- When OFI owner is changed.
- When OFI becomes in progress status.
- When a task is created and linked to OFI
- When a compliance control is linked to OFI.
- When a compliance conttrol is unliked from and OFI, Risk or Incident.
- When a task is created in task manager.
- When is task is completed in task manager.
- When an assigned task is accepted or denied by a user.
- When a task becomes in progress status.
- When an ai assisted analysis is saved for a risk.
- When an ai assisted analysis is saved for an incident.
- When a saved analysis is deleted from a risk.
- When a saved analysis is delete from an incident.
...
- Capture activity when risk created.
- Capture activity when risk mitigated.
- Capture activity when risk escalated.
- Capture activity when risk accepted.
- Capture activity when owner is changed.

### Incident Management

- Capture activity when incident created.
- Capture activity when incident escalated.
- Capture activity when incident resolved.
- Capture activity when owner is changed.

### Common

- Capture activity when iso-control is linked
- Capture activity when a task is linked.
- Capture activity when a attachment is linked.
- Capture activity when someone is nudged
