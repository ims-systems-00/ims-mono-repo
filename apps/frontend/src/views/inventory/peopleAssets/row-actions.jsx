...
export const RowActions = ({ row }) => {
  ...
  let { processing, handlePeopleDelete } = usePeopleAssets();
  const { isOpen: isDeleteModalOpen, toggle: toggleDeleteModal } =
    useDualStateController();
  return (
    <React.Fragment>
      <TourStep data-tour-step="people-row-actions">
        <DTRowActionsDropdown>
          ...
          <DTRowActionsMenu>
            <DTRowAction
              onClick={(e) => {
                e.stopPropagation();
                history.push(`/admin/inventory/people/${row?.original?._id}`);
              }}
            >
              Details
            </DTRowAction>
        ...
        >
          ...
          <ModalBody>
            ...
            <div className="text-center mt-2">
              ...
              <Button
                onClick={() => handlePeopleDelete(row?.original)}
