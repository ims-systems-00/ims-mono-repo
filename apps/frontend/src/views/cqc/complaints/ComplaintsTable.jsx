...
import { deleteComplaint } from "@/services/cqcServices";
...
const ComplaintsTable = ({
  ...
  let updateDataTable = (updatedComplaint) => {
    setProcessing({ action: "update", id: updatedComplaint._id });
  ...
  async function handleTableButton(e, complaint) {
    try {
      let actionType = e.target.name;
      let complaintId = complaint._id;
      switch (actionType) {
        case "delete": {
          setProcessing({ action: "delete", id: complaintId });
      ...
      imsLogger("ComplaintTable", ex.response || ex);
                    ...
                    <DropdownItem
                      onClick={(e) => {
                        warningWithConfirmMessage(
                          "This complaint will be deleted",
                          () => {
  ...
  return (
    <>
      ...
      <div className="content">
        ...
        />
        <Modal title="Complaint">
          <ComplaintDetails isModalOpen={isOpen} />
        </Modal>
      </div>
    </>
  );
