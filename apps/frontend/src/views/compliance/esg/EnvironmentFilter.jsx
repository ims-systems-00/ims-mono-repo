...
import environmentalFilters from "./environmentalSections";
  ...
  return (
    <>
      <Button
        ...
        onClick={toggleEnvironmentalModalFilter}
      >
        <i class="ims-icons icon-icon-funnelsimple-24 me-1 p-0" />
        Filter
      </Button>
      <Modal
        isOpen={environmentalModalFilter}
        toggle={toggleEnvironmentalModalFilter}
        style={{ width: "600px" }}
        backdrop={processing[LOADER.LOAD_ENVIRONMENTAL].status ? false : true}
      >
        <ModalHeader toggle={toggleEnvironmentalModalFilter}>
          <h3 className="text-dark">Filter</h3>
        </ModalHeader>
        <ModalBody>
          <Form action="/" className="form-horizontal" method="get">
            <p>Filter environmental to get specific data</p>
            <Row className="mt-3">
              <Col md={"12"}>
                <ImsInputSelect
                  label={"Status"}
                  ...
                  options={environmentalFilters.map((filter) => ({
                    value: filter.value,
                    label: filter.label,
                  }))}
                />
                ...
                <Row>
                  ...
                  >
                    ...
                    <Button
                      ...
                      className="shadow-sm--hover"
                      onClick={() => {
                        EnvironmentalQueryTools?.handleFilter({
                          value: {
