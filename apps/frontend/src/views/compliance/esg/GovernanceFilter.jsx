...
import governanceFilters from "./governanceSections";
  ...
  return (
    <>
      <Button
        ...
        onClick={toggleGovernanceModalFilter}
      >
        <i class="ims-icons icon-icon-funnelsimple-24 me-1 p-0" />
        Filter
      </Button>
      <Modal
        isOpen={governanceModalFilter}
        toggle={toggleGovernanceModalFilter}
        style={{ width: "600px" }}
        backdrop={processing[LOADER.LOAD_GOVERNANCE].status ? false : true}
      >
        <ModalHeader toggle={toggleGovernanceModalFilter}>
          <h3 className="text-dark">Filter</h3>
        </ModalHeader>
        <ModalBody>
          <Form action="/" className="form-horizontal" method="get">
            <p>Filter governance to get specific data</p>
            <Row className="mt-3">
              <Col md={"12"}>
                <ImsInputSelect
                  label={"Status"}
                  ...
                  options={governanceFilters.map((filter) => ({
                    value: filter.value,
                    label: filter.label,
                  }))}
                />
                ...
                <Row>
                  ...
                  >
                    ...
                    <Button
                      ...
                      className="shadow-sm--hover"
                      onClick={() => {
                        GovernanceQueryTools?.handleFilter({
                          value: {
...
export default GovernanceFilter;
